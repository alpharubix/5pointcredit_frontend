  ...
  return (
    <div className="bg-white rounded-md shadow-sm border border-gray-200 mb-6 overflow-hidden animate-in fade-in duration-500">
      <div className="overflow-auto max-h-[70vh]">
        ...
        <div className="sticky top-0 z-50 h-10 flex items-center justify-center bg-[#e67e22] text-white font-semibold">
          {title}
        </div>
        <table className="w-full text-sm text-left border-separate border-spacing-0">
          ...
          <thead className="bg-[#000080] text-white">
            <tr>
              <th className="sticky top-10 left-0 z-40 h-12 bg-[#000080] px-4 align-middle border border-[#000080]">
                Particulars
              </th>
              {years.map(y => (
                <th
                  key={y}
                  className="sticky top-10 z-30 h-12 bg-[#000080] px-4 align-middle border border-[#000080] text-right min-w-[120px]"
                >
                  {y}
                </th>
              ))}
          ...
          <tbody>
            {allKeys.map((key, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                <td
                  className={`sticky left-0 z-20 bg-white px-4 py-2 border border-gray-200 ${
                    isHighlighted(key)
...
export const renderDataTable = (title: string, dataArray: any[], nested: boolean = false) => {
  if (!dataArray || dataArray.length === 0) return null;
  ...
  if (nested) {
    return (
      <div className="mb-2">
        <div className="bg-[#000080] text-white px-4 py-2 text-center font-semibold opacity-90">
          {title}
        </div>
        {table}
      </div>
